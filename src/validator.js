 ...
 * @param {*} validator
 */
module.exports.applyRules = function apply(field, validator) {
  let proceed = true;
  ...
  for (let r = 0; r < field.rules.length; r++) {
    ...
    const result = validationRules[rule].apply(validator, ruleArgs);
 ...
 * @param {*} validator
 */
module.exports.applyPostRules = function postApply(rule, validator) {
  if (rule.rule == 'function') {
    return rule.params.apply(validator, [rule.values]);
  }

  return postValidationRules[rule.rule].apply(validator, [rule.values, rule.params]);
};
